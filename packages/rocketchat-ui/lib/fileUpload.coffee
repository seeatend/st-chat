readAsDataURL = (file, callback) ->
	...
	if org?.hasActiveSubscription
		maxSize = RocketChat.settings.get('FileUpload_MaxFileSize_ForPremuim')
	else maxSize = RocketChat.settings.get('FileUpload_MaxFileSize')

	if file.size > maxSize
		swal
			title: t('FileUpload_MaxFileSize_MaxAllowedSize', maxSize/1048576) #convert to MB
			type: 'error'
			html: true
		return
