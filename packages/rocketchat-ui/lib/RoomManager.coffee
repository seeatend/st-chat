		...
		removed: (record) ->
			if RoomManager.getOpenedRoomByRid(record.rid)?
				...
				if recordAfter?
					...
					recordAfter.timeLine = RocketChat.services.getMessageTimeLine(recordAfter, recordBefore)
					Meteor.call 'updateMessage', recordAfter
...
@RoomManager = new class
	...
	getRoomName = (rid) ->
		roomData = ChatRoom.findOne({ _id: rid})
		roomName = ''

		if roomData
			if roomData.t is 'd'
				roomName = ChatSubscription.findOne({ 'u._id': Meteor.userId(), rid: rid }, { fields: { receiverName: 1 } })?.receiverName
			else if roomData.t is 't'
				room = ChatSubscription.findOne({ 'u._id': Meteor.userId(), rid: rid }, { fields: { fullName: 1, mrn: 1 } })
				roomName = if (room.fullName and room.mrn) then room.fullName + ' #' + room.mrn else roomData.name
			else
