class @ChatMessages
	...
	bindEvents: ->
		isInside = (element, targetClasses) ->
			...
			return isInside($(element).parent(), targetClasses)

		that = this

		$('body').click (e)->
			if !isInside(e.target, ['edit-message', 'message-form', 'sweet-alert'])
				that.clearEditing()
	...
	keydown: (rid, event) ->
		...
		k = event.which
		this.resize(input)
		if k is 13 and Meteor.isCordova
			return
		else if k is 13 and not event.shiftKey
