Template.adminManageUsers.helpers
	isReady: ->
...
Template.adminManageUsers.onCreated ->
	instance = @
...
Template.adminManageUsers.onRendered ->
...
Template.adminManageUsers.events
...
Template.adminManageUser.helpers
	isAdmin: ->
		return false
...
Template.adminManageUser.events
	'click .delete-user': ->
		userId = this._id
		userName = this.name
		swal {
			...
			showCancelButton: true
			closeOnConfirm: false
			confirmButtonText: 'Delete ' + userName
			confirmButtonColor: '#ec6c62'
		}, ->
			Meteor.call 'deleteUser', userId, (error, result) ->
				if error
					...
					swal
						title: 'User deleted!'
						text: 'The user ' + userName + ' has been successfully removed'
						type: 'success'
						html: true
